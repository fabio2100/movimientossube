...
export default function Main() {
  ...
  useEffect(() => {
    ...
    arrayProvisorio.forEach((item) => {
      ...
      setBalancesMes2([
        ...balancesMes2,
        item.ValueFormat.slice(2).replace(",", "."),
      ]);
  ...
  return (
    <>
      ...
      {Object.keys(allMesData.objEvolucionSaldo).length !== 0 &&<div className="lineChart">
        <h2>Evolución del saldo</h2>
        <LineChart
          grid={{ vertical: true, horizontal: true }}
          xAxis={[{ data: allMesData.objEvolucionSaldo.evolucionFechasSaldo}]}
          series={[
            {
              data: allMesData.objEvolucionSaldo.evolucionSaldos,
            },
          ]}
          width={1500}
          height={300}
